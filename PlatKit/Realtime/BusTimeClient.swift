...
public actor BusTimeClient {
    ...
    public func arrivals(forStopCode stopCode: String,
        ...
        // Hit our Cloudflare Worker; it injects the MTA Bus Time API key
        // server-side and forwards to bustime.mta.info. The key never ships
        // in the iOS binary. See workers/bus-proxy/.
        var comps = URLComponents(url: AppConfig.busProxyURL.appendingPathComponent("api/siri/stop-monitoring.json"),
